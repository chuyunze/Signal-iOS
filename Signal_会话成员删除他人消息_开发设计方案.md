...
enum ParticipantDeleteConfiguration {
    ...
    static let featureEnabled = true // 当前测试构建直接开启
}
```
...
`expectedDeviceSnapshot` 只有在服务端/自有基础设施能提供发送时设备快照时才写入；为空时只能计算已有设备确认，不能计算“全部预期设备确认”。
...
这里必须区分：服务端“向账号投递成功”、某一设备返回执行回执，以及全部预期设备返回执行回执。只有最后一种状态需要预期设备集合；当前纯客户端实现默认只能可靠显示前两种。
...
- 删除后生成的兼容备份必须保存消息墓碑、删除者展示信息和稳定 `MessageTargetKey`；
- `ParticipantDeleteTombstone` 作为独立删除账本导出，恢复时先导入删除账本，再导入/渲染普通消息；
- 墓碑优先级高于原消息和任何编辑版本；
...
因此本方案默认的“备份恢复不复活”准确含义是：**从删除完成之后生成、且包含 Participant Delete 墓碑/删除账本的兼容备份恢复时，原内容不得复活。**
...
当前引用快照嵌在消息模型中，没有面向目标键的独立索引。若验收要求删除后主动清空所有既有引用卡片，不能在每次删除时无界扫描全库；应新增引用目标索引，或设计有上限、可续跑的后台清理任务。仅清理目标消息自身的 `quotedMessage` 字段不能覆盖其他消息中保存的引用快照。
